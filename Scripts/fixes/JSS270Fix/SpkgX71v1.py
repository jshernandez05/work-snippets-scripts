#!/usr/bin/python
"""
Recreate X71 from malsp transfer.

The following script is an intermediate fix for the JSS270 batch process.
The X71s are parsed from the JSS270 input file and then inserted to the
malsp_rcpt table via SQL statement.  
"""
# Libraries used
...
"""
Last JSS270 batch job.

The following variables are used to identify the batch number of the latest JSS270 batch job.
The latest batch number is compared to the batch number in the jxhist file to identify 
whether the script has been previously run for the latest batch number.  The jxhist file is
written to at the completion of the script to ensure the script is not ran multiple times
against the same batch job number.
"""
...
if os.path.exists(filepath) and latest_file != '' and lastbatch != batch:
    ...
    with open(filepath, 'r') as rf:
        ...
        with open(pkickoff, 'w') as kf:  # creates the bash script that runs the insert to iSQL
