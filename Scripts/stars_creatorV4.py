...
ARCH = subprocess.Popen("uname -p", shell=True, stdout=subprocess.PIPE).communicate()[0].strip()
if ARCH == "x86_64":
    # 3.40
    CREATE_PD = "/h/NTCSSS/bin/getlinkdata -q ssabtusr > {0}/pd".format(TEMP_DIR)
else:
    # 3.30
    CREATE_PD = "/h/NTCSSS/bin/getlinkdata rsup get | grep 'Second value' | sed -e 's/^.*= //' > {0}/pd".format(TEMP_DIR)
...
class Validator(BaseFileHandler):
    ...
    SQL_TEMPLATE = (
                ...
                "union\n"
                "(select '{0}FY: ' + rtrim(cast(max(fscl_yr) as char)) from {0})\n"
                "union\n"
                "(select '{0}TL: ' + rtrim(cast(max(trnsmtl_no) as char)) from {0} where fscl_yr = (select max(fscl_yr) from {0}))\n"
                "union\n"
    ...
    def __init__(self, bor_tbl, is_monthly):
        """
    ...
    def create_pd_file(self):
        ...
        self.set_permissions(PD, 0755, 1000, 1000)
    ...
    def generate_sql_script(self):
        ...
        sql_text = self.SQL_TEMPLATE.format(self.bor_tbl)

        self.write_file(OUTFILE, sql_text)
        self.set_permissions(OUTFILE, 0755, 1000, 1000)
    ...
    def generate_pkickoff_script(self, sybase_ocs_path):
        ...
        """
        pkickoff_text = (
            "{0}/bin/isql -Ussabtusr -P$(cat {1}) "
            "-SNTCSS_RSUP -Dsupply -b -i {2} -o {3}".format(
                sybase_ocs_path, PD, OUTFILE, VALFILE
            )
        )

        self.write_file(PKICKOFF, pkickoff_text)
        self.set_permissions(PKICKOFF, 0777, 1000, 1000)
    ...
    def execute_pkickoff_script(self):
        ...
        """
        subprocess.call('su - sybase -c {0}'.format(PKICKOFF), shell=True)
...
class LineParser:
    ...
    def parse_line(line, format_key):
        ...
        if format_key == "X0A":
            parsed_data["fmv"] = abs(int(mv_raw.strip())) if mv_raw.strip().isdigit() else 0
            parsed_data["smv"] = str(parsed_data["fmv"]).zfill(10)
        elif format_key == "B1N":
            parsed_data["fmv"] = abs(int(mv_raw.lstrip("0"))) if mv_raw.lstrip("0").isdigit() else 0
...
class StarsFileCreator(BaseFileHandler):
    ...
    def ensure_directories(self):
        ...
        if not os.path.exists(TEMP_DIR):
            os.makedirs(TEMP_DIR)
            self.set_permissions(TEMP_DIR, 0777, 1000, 1000)
        else:
            self.set_permissions(TEMP_DIR, 0777, 1000, 1000)
    ...
    def get_rsup_cfg_lvl(self):
        ...
        """
        cmd = "cat /etc/hosts | grep nalc | awk '{print $3}'"
    ...
    def run_stars_creator(self):
        ...
        if self.validate:
            validator = Validator(self.bor_tbl, self.is_monthly)
        ...
        self.set_permissions(outfile, 0660, os.geteuid(), 1001)
        self.cleanup()
