...
ARCH = subprocess.Popen(["uname","-p"], shell=False, stdout=subprocess.PIPE, text=True).communicate()[0].strip()
...
class Validator(BaseFileHandler):
    ...
    def create_pd_file(self):
        ...
        if ARCH == "x86_64":
            ...
            with open(PD, "w") as wf:
                subprocess.call(cmd, stdout=wf, stderr=subprocess.PIPE, shell=False, text=True)
        else:
            # 3.30
            ...
            with open(PD, "w") as wf:
                subprocess.call(["tee"], stdin=sed_proc.stdout, stdout=PD, stderr=subprocess.PIPE, text=True)
    ...
    def execute_pkickoff_script(self):
        ...
        """
        subprocess.call(["su", "-", "sybase", "-c", "{0}".format(PKICKOFF)], shell=False)
...
class StarsFileCreator(BaseFileHandler):
    ...
    def get_rsup_cfg_lvl(self):
        ...
        """
        cmd = ["awk", "/nalc/ {print $3}", "/etc/hosts"]
        process = subprocess.Popen(cmd, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, _ = process.communicate()
        stdout = stdout.strip()

        return "force" if stdout else "unit"
