...
def get_valid_app_groups(default_apps, group_records):
    ...
    for app in default_apps:
        ...
        if any(gr.gr_name == '{0}01'.format(app) for gr in group_records):
            app_grps.append(app)

    return app_grps


def get_all_groups(app_groups, group_records):
    """
    Retrieve all groups and their members for the specified PAM groups.

    Args:
        app_groups (list): A list of valid PAM group group base names.
        group_records (list): Cached result of `grp.getgrall()`.
    ...
    """
    groups_data = {}  # Dictionary to store group information

    for basename in app_groups:
        # Create a regex pattern to match group names based on the basename
        if basename == "ntcss":
            pattern = re.compile(r"^{basename}\d{{2}}$".format(basename=basename))
        else:
            pattern = re.compile(r"^{basename}(\d{{2}})?$".format(basename=basename))
        # Collect matching groups and their members
        groups_data[basename] = dict(
            [(gr.gr_name, gr.gr_mem) for gr in group_records if pattern.match(gr.gr_name)]
        )
    return groups_data
...
def main():
    ...
    # Get all group records from passwd
    try:
        group_recs = grp.getgrall()  # Get all group records
    except Exception as e:
        print("ERROR: Failed to fetch group records: {0}\n".format(str(e)))

    # Get valid PAM groups for site
    app_grps = get_valid_app_groups(DEFAULT_APPS, group_recs)

    if not app_grps:
        print("ERROR: No valid PAM groups found. Exiting.\n")
        sys.exit(1)

    # Get all PAM groups and their users
    groups_data = get_all_groups(app_grps, group_recs)
    ...
    # group_ct_before, user_ct_before = get_group_counts(groups_data)
    ...
    try:
        ...
        if not user_movements:
            ...
            updated_group_recs = grp.getgrall()
            updated_groups = get_all_groups(app_grps, updated_group_recs)
        ...
        print("\nGroup Counts After")
        print("==================")
        group_ct_after, user_ct_after = get_group_counts(updated_groups)
