if object_id ('vw_impr_cur_mtd') is not null

begin
drop view vw_impr_cur_mtd
end
go
...
select
	...
	'Rel.2' as 'Rel',
	convert(char(10), convert(varchar, dateadd(dd, -(day(getdate()) - 1), getdate()), 101)) as 'start_dt',
	convert(char(10), convert(varchar, getdate(), 101)) as 'end_dt',
...
from dr09_requisition
	...
	join da03_site on da03_site.aimd_org = da01_organizatn.org_cd
where
	dr09_requisition.ord_dttm >= convert(varchar, dateadd(dd, -(day(getdate()) - 1), getdate()), 101)
	and dr09_requisition.ord_dttm <= convert(varchar, getdate(), 101)
...
grant select on vw_impr_cur_mtd to public
go
